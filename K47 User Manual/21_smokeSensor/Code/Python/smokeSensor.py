...
LedPin = 16					# Set buzzer pin
threshold   = 100			# You can set the Threshold by yourself (0-255)

def setup():
	GPIO.setmode(GPIO.BOARD)		# Numbers GPIOs by physical location
	GPIO.setup(LedPin, GPIO.OUT)		# Set pins' mode is output
...
def loop():
	while True:
		...
		if tmp > threshold :						# LedPin when read value greater than threshold
			print '    ****************'
			print '    * ! DANGER ! *'
			print '    ****************'
			print ''
				
			GPIO.output(LedPin, GPIO.HIGH)	# (0, means detect danger gas)
			time.sleep(0.25)
